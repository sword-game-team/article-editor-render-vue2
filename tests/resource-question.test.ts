...
describe('controlled visibility and snapshot rendering', () => {
  ...
  it('notifies stable IDs without auto-reveal and restores hiding when keys are removed', async () => {
    ...
    const { wrapper, selected, update } = render(doc, { resourceQuestionFooterText: '<em>Host text</em>\nSecond line' })
    expect(wrapper.find('fieldset,legend,em').exists()).toBe(false)
    expect(wrapper.find('.acp-resource-question').element.tagName).toBe('DIV')
    expect(wrapper.find('.acp-resource-question__description').text()).toBe('Saved description')
    expect(wrapper.findAll('.acp-resource-question__footer').length).toBe(1)
    expect(wrapper.find('.acp-resource-question__footer').text()).toBe('<em>Host text</em>\nSecond line')
    ...
    expect(wrapper.findAll('.acp-resource-question__footer').length).toBe(2)
    await update({ resourceQuestionFooterText: 'Updated footer' })
    expect(wrapper.findAll('.acp-resource-question__footer').wrappers.map((footer) => footer.text())).toEqual(['Updated footer', 'Updated footer'])
    await update({ revealedKeys: [] }); expect(wrapper.text()).not.toContain('middle')
    expect(wrapper.findAll('.acp-resource-question__footer').length).toBe(1)
    await update({ resourceQuestionFooterText: ' \n ' })
    expect(wrapper.find('.acp-resource-question__footer').exists()).toBe(false)
  ...
  it('uses plain snapshot text and renders all content if hideFollowing is false', () => {
    ...
    expect(wrapper.find('.acp-resource-question__description').text()).toBe(q.attrs.description)
  })
  ...
  it('reports identity errors even in tolerant mode, but preserves strict documents with stale bindings', async () => {
    ...
    expect(warning.wrapper.find('.acp-resource-question').element.tagName).toBe('DIV')
